...
- **Image**: A query about an image.
- **Document**: A query about a PDF document.
...
```python
from renforce import AIOrchestrator, Renforce
...
Below is an example of using `AIOrchestrator` to query `gpt-4o-mini` and ask a question about a document.

``` python

from renforce import AIOrchestrator, Renforce
from renforce.data import RenDoc
from pathlib import Path
...
# Create a RenDocs
doc_1 = RenDoc(
	data=Path("astronomy.pdf"),
	description="A document about the astronomy"
)

doc_2 = RenDoc(
	data=Path("galaxies.pdf"),
